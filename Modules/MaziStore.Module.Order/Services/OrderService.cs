...
namespace MaziStore.Module.Orders.Services
{
   public class OrderService : IOrderService
   {
      ...
      public async Task<MaziResult<Order>> CreateOrder(
         long cartId,
      ...
      )
      {
         ...
         if (cart is null)
         {
            return MaziResult.Fail<Order>($"Cart id {cartId} cannot be found.");
         }

         var shippingData = JsonSerializer.Deserialize<DeliveryInformationVm>(
            cart.ShippingData
         );
         ...
         else if (shippingData.BillingAddressId == 0)
         {
            var address = new Address
            {
               ...
               StateOrProvinceId = shippingData
         ...
         {
            billingAddress = _userAddressRepository
               .QueryRp()
               .Where(x => x.Id == shippingData.BillingAddressId)
               .Select(x => x.Address)
               .First();
         }
      ...
      )
      {
         ...
         {
            return MaziResult.Fail<Order>(checkingDiscountResult.ErrorMessage);
         }

         var validateShippingMethodResult = await ValidateShippingMethod(
            shippingMethodName,
            shippingAddress,
            cart
         );
         ...
         {
            return MaziResult.Fail<Order>(validateShippingMethodResult.Error);
         }
         ...
         {
            if (
               !cartItem.Product.IsAllowToOrder
               || !cartItem.Product.IsPublished
               || cartItem.Product.IsDeleted
            )
            {
               return MaziResult.Fail<Order>(
                  $"The product {cartItem.Product.Name} is not available any more"
               );
            }
            ...
            var taxPercent = await _taxService.GetTaxPercent(
               cartItem.Product.TaxClassId,
            ...
            var discountedItem =
               checkingDiscountResult.DiscountedProducts.FirstOrDefault(
         ...
         order.SubTotalWithDiscount =
            order.SubTotal - checkingDiscountResult.DiscountAmount;
         order.OrderTotal =
            order.SubTotal
            + order.TaxAmount
         ...
         _orderRepository.AddRp(order);

         cart.IsActive = false;
         var vendorIds = cart.Items
         ...
         foreach (var vendorId in vendorIds)
         {
            ...
            foreach (
               var cartItem in cart.Items.Where(x => x.Product.VendorId == vendorId)
            )
            {
               var taxPercent = await _taxService.GetTaxPercent(
                  cartItem.Product.TaxClassId,
            ...
            subOrder.SubTotal = subOrder.OrderItems.Sum(
            ...
            subOrder.OrderTotal =
               subOrder.SubTotal
               + subOrder.TaxAmount
               + subOrder.ShippingFeeAmount
               - subOrder.DiscountAmount;
            _orderRepository.AddRp(subOrder);
            subOrders.Add(subOrder);
         }

         using (var transaction = _orderRepository.BeginTransactionRp())
         {
            _orderRepository.SaveChangesRp();
            ...
            _couponService.AddCouponUsage(
               cart.CustomerId,
               order.Id,
               checkingDiscountResult
            );
            _orderItemRepository.SaveChangesRp();
            transaction.Commit();
         }

         await _mediator.Publish(new AfterOrderCreated(order));

         return MaziResult.Ok(order);
      }
      ...
      )
      {
            ...
            .Select(
               x =>
                  new CartItemVm
                  {
                     ...
                     TaxClassId = x.Product.TaxClassId
                  }
            )
            .ToList();
         ...
         foreach (var cartItem in cartItems)
         {
            ...
            {
               var taxRate = await _taxService.GetTaxPercent(
                  cartItem.TaxClassId,
                  countryId,
                  stateOrProvinceId,
                  zipCode
               ...
               taxAmount =
                  taxAmount + cartItem.Quantity * cartItem.Price * taxRate / 100;
            }
         }
      ...
      public async Task<OrderTaxAndShippingPriceVm> UpdateTaxAndShippingPrices(
         long cartId,
      ...
      )
      {
         ...
         {
            address = await _userAddressRepository
               .QueryRp()
               .Where(x => x.Id == model.ExistingShippingAddressId)
               .Select(x => x.Address)
               .FirstOrDefaultAsync();
            if (address == null)
            {
               throw new ApplicationException(
                  $"Address id {model.ExistingShippingAddressId} not found."
               );
            }
         }
         else
         ...
         {
            Cart = await _cartService.GetActiveCartDetails(
               cart.CustomerId,
               cart.CreatedById
            )
         };

         cart.TaxAmount = orderTaxAndShippingPrice.Cart.TaxAmount = await GetTax(
            cartId,
            address.CountryId,
            address.StateOrProvinceId,
            address.ZipCode
         );
         ...
         orderTaxAndShippingPrice.ShippingPrices =
            await _shippingPriceService.GetApplicableShippingPrices(request);

         var selectedShippingMethod = string.IsNullOrWhiteSpace(
            model.SelectedShippingMethodName
         )
         ...
         {
            cart.ShippingAmount = orderTaxAndShippingPrice.Cart.ShippingAmount =
               selectedShippingMethod.Price;
            cart.ShippingMethod =
               orderTaxAndShippingPrice.SelectedShippingMethodName =
      ...
      private async Task<MaziResult<ShippingPrice>> ValidateShippingMethod(
         string shippingMethodName,
         Address shippingAddress,
         Cart cart
      )
      {
         var applicableShippingPrices =
            await _shippingPriceService.GetApplicableShippingPrices(
         ...
         var shippingMethod = applicableShippingPrices.FirstOrDefault(
            x => x.Name == shippingMethodName
         );

         if (shippingMethod == null)
         {
            return MaziResult.Fail<ShippingPrice>(
               $"Invalid shipping method {shippingMethod}"
            );
         }

         return MaziResult.Ok(shippingMethod);
      }
   }
}
